...
class SyntheticImage(ABC):
    ...
    def project_point_copy(self, y_points):
        ...
        for ypt in y_points['coordinates']:
            ...
            ypt_x.append(ypt_2d_asec.Tx.value)
            ypt_y.append(ypt_2d_asec.Ty.value)
        
        ypt_2d_dict = {'x': np.array(ypt_x), 'y': np.array(ypt_y)}

        map_ypoints_coords = []
        for x, y in zip(ypt_2d_dict['x'], ypt_2d_dict['y']):
            ypt_coord = SkyCoord(x*u.arcsec, y*u.arcsec, frame=self.synth_map.coordinate_frame)
            ypt_coord_pix = self.ref_img.wcs.world_to_pixel(ypt_coord)

            scalex = self.ref_img.scale[0].value
            scaley = self.ref_img.scale[1].value

            x_shifted = x + self.image_shift[0] * scalex * self.zoom
            y_shifted = y + self.image_shift[1] * scaley * self.zoom
    ...
    def project_point(self, y_points):
        ...
        tr_coord_x = m.top_right_coord.Tx
        tr_coord_y = m.top_right_coord.Ty
        tr_pix_x = (tr_coord_x / m.scale[0]).value
        tr_pix_y = (tr_coord_y / m.scale[1]).value

        ref_coord_x = m.reference_coordinate.Tx
        ref_coord_y = m.reference_coordinate.Ty
        ref_pix_x = (ref_coord_x / m.scale[0]).value
        ref_pix_y = (ref_coord_y / m.scale[1]).value
        ...
        for ypt in y_points['coordinates']:
            ...
            # 1. Bottom left pixel
            # 2. Image shift
            # 3. (0,0) centered pixels scaled by zoom
            # x_shifted = (
            #             #  bl_pix_x 
            #             #  + self.image_shift[0]
