...
def semi_circle_loop(radius, majax, minax, ellipse, height, theta0=0 * u.deg, phi0=0 * u.deg, el=90 * u.deg, az=0 * u.deg, samples_num=100):
    ...
    # LOOP LENGTH NEEDS TO CHANGE FOR ELLIPSE (not circle)
...
class CoronalLoopBuilder:
    ...
    def height(self, value):
        # Needs to be adjusted if majax or minax is being used
    ...
    def _init_sliders(self):
        ...
        def submit_majax(text):
            self.from_textbox = True
            self.slider_majax.set_val(float(text))
            self.from_textbox = False
            self._update(None)

        def submit_minax(text):
            self.from_textbox = True
            self.slider_minax.set_val(float(text))
            self.from_textbox = False
            self._update(None)
