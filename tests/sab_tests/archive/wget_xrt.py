...
def con_pre(year, month, day, hour):
    ''' Constructs preamble for xrt_lvl_0 image using date time info '''
...
def txt2url(textfile, year, month, day):

    # https://data.darts.isas.jaxa.jp/pub//hinode/xrt/level0/2013/05/15/H0400/XRT20130515_040058.0.fits
    textfile = textfile if textfile.endswith('.txt') else textfile + '.txt'
    dir = '/home/saber/Downloads/'
    path = dir + textfile
    ...
    for line in lines:
        line = line.split('\n')[0]
        hour = line.split('_')[1][0:2]
        preamble = con_pre(year, month, day, hour)
        urls.append(preamble + line)

    return urls


textfile = '2013_xrt_fnames.txt'
urls = txt2url(textfile, 2013, 5, 15)

# print(urls)

# Make event directory environmental variable in bash
directory = '/media/saber/My_Passport/Sabastian/xrt/2013_05_15/xrt_lvl_0'
options = '-nv -m --force-directories -nH --cut-dirs=2 -P {}'.format(directory)

# Searches database for each url
